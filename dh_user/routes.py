...
@router.post(
    '/block', description='Блокирует пользователя', response_model=SimpleOperationResult
)
async def block(
        payload: DeleteOrBlockUserIn, user: UserModel = Depends(get_current_user)
) -> dict[str, bool]:
    """Блокировка"""
...
@router.post(
    '/delete', description='Помечает пользователя на удаление', response_model=SimpleOperationResult
)
